...
public class UsuarioController {
	...
	@PutMapping
	public ResponseEntity<Usuario> editarUsuario(@Valid @RequestBody Usuario user) {
		return ResponseEntity.status(201).body(usuarioService.editarUsuario(user));
	}
	...
	@PostMapping("/login")
	public ResponseEntity<Token> logar(@Valid @RequestBody UsuarioDto user) {
		Token token = usuarioService.gerarToken(user);
		if (token != null) {
